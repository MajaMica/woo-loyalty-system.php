...
function display_vatrice_message_on_thankyou_page( $order_id ) {
    if ( ! $order_id ) return;
    $order = wc_get_order( $order_id );
    $user_id = $order->get_user_id();
    if ( ! $user_id ) return;
    ...
    if ( $new_vatrice > 0 ) {
        echo '<div class="thankyou-vatrice">';
        echo '<h2>🎉 Čestitamo!</h2>';
        echo '<p>Osvojili ste <strong>' . esc_html( $new_vatrice ) . ' 🔥 vatrica</strong>!</p>';
        echo '<p>Možete ih iskoristiti u sledećoj kupovini – 1 vatrica = 1 RSD popusta.</p>';
        echo '</div>';
    }
}
add_action( 'woocommerce_thankyou', 'display_vatrice_message_on_thankyou_page', 5 );
